...
function generateImage() {
    ...
    if (!text) {
        showNotification('请输入要转换的文字', 'error');
        return;
    }
    ...
    setTimeout(() => {
        try {
            ...
            showNotification('生成图片时出现错误，请重试', 'error');
        } finally {
            // Reset button state
...
function downloadImage() {
    ...
    try {
        ...
        showNotification('下载图片时出现错误，请重试', 'error');
    }
}
...
window.addEventListener('load', function() {
    ...
    setTimeout(() => {
        if (!textInput.value) {
            textInput.value = '欢迎使用文字转图片工具！\n\n在这里输入您想要转换的文字，选择字体、颜色和背景，然后点击生成按钮即可创建精美的图片。\n\n支持多种格式下载，完全免费使用！';
            handleTextInput();
        }
    }, 2000);
});
